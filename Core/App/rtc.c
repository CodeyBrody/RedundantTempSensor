...
{
    return ((usartMessage[firstDigit] - '0') * 10 + (usartMessage[secondDigit] - '0'));
}
...
uint8_t SakamotoAlgo(RTC_DateTypeDef sDate)
{
    ...
    return ((sDate.Year + sDate.Year / 4 - sDate.Year / 100 + sDate.Year / 400 +
             t[sDate.Month - 1] + sDate.Date) %
            7);
}
...
uint8_t RTC_checkTimeInput(const char *input, RTC_DateTypeDef *sDate, RTC_TimeTypeDef *sTime)
{
    ...
    if (input[4] != '/' || input[7] != '/' || input[10] != ' ' || input[13] != ':' ||
        input[16] != ':')
    {
        return 1;
    }
...
uint8_t RTC_SetTime(void)
{
    ...
    {
        // If RTC_checkTimeInput returned 1, then a formatting error was encountered...pass it along
        // by also returning 1
        if (RTC_checkTime_return == 1)
        {
            return 1;
        }
        else if (RTC_checkTime_return == 2)
        { // If it returned 2, pass along an invlaid date/time error by also returning 2
            return 2;
        }
        else
        { // Unrecognized return code received
            return -1;
        }
    }

    // If the program proceeds here, should be formatted correctly. Now to set the time and check
    // the return type.
