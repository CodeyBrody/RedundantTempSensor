...
uint8_t discoverSensorArray(uint8_t SensorCount, Sensor sensors[]){
    ...
    for(int i = 0; i < SensorCount; i++){
        ret = HAL_I2C_Master_Transmit(&hi2c1, TMP102_ADDRESSES[i], I2CBuf, 1, I2C_MAX_DELAY);
...
float readTempSensor(Sensor *s){
    ...
    ret = HAL_I2C_Master_Transmit(&hi2c1, s->address, I2CBuf, 1, I2C_MAX_DELAY);
    if(ret != HAL_OK){ //If we get anything other than HAL_OK, skip next steps...
      ...
      ret = HAL_I2C_Master_Receive(&hi2c1, s->address, I2CBuf, 2, I2C_MAX_DELAY);
